...
rangy.createModule("DomUtil", function (l, K) {
	...
	function I(c, f) {
		...
		for (r = f; r; r = r.parentNode) if (v(k, r)) return r;
		return null
	}
	function z(c, f, k) {
	...
	var v = function (c, f) {
		for (var k = c.length; k--;) if (c[k] === f) return true;
		return false
	};
	n.prototype = {
		_current: null,
		...
		next: function () {
			var c = this._current = this._next,
				f;
			if (this._current) if (f = c.firstChild) this._next = f;
				else {
	...
	l.dom = {
		...
		isAncestorOf: function (c, f, k) {
			for (f = k ? f : f.parentNode; f;) if (f === c) return true;
				else f = f.parentNode;
			return false
		},
...
rangy.createModule("DomRange", function (l) {
	...
	function I(a, e, j) {
		if (e = a._listeners[e]) for (var o = 0, E = e.length; o < E; ++o) e[o].call(a, {
					target: a,
					args: j
				})
	}
	...
	function i(a, e, j) {
		...
		for (j = j || {
			stop: false
		}; o = a.next();) if (a.isPartiallySelectedSubtree()) if (e(o) ===
					false) {
					j.stop = true;
					return
				} else {
					...
					o.detach(true);
					if (j.stop) return
				} else for (o = g.createIterator(o); E = o.next();) if (e(E) === false) {
							j.stop = true;
							return
						}
	}
	function n(a) {
		for (var e; a.next();) if (a.isPartiallySelectedSubtree()) {
				e = a.getSubtreeIterator();
				n(e);
				e.detach(true)
			} else a.remove()
		...
		l.util.extend(a.prototype, {
			...
			normalizeBoundaries: function () {
				...
				if (g.isCharacterDataNode(y)) y.length ==
					...
					ga = !this.collapsed
				} if (ga) if (g.isCharacterDataNode(m)) s == 0 && qa(m);
					else {
...
rangy.createModule("WrappedSelection", function (l, K) {
	...
	function O(b) {
		...
		a: if (!d.length || d[0].nodeType != 1) h = false;
			...
			h = 1;
			for (var D = d.length; h < D; ++h) if (!c.isAncestorOf(d[0],
					d[h])) {
					h = false;
					break a
				}
	...
	function v(b) {
		...
		if (typeof b.rangeCount != "undefined") for (var P = 0, X = b.rangeCount; P < X; ++P) d[P] = k.inspect(b.getRangeAt(P));
		...
		g.addRange = fa ? function (b, d) {
			if (M && V && this.docSelection.type == "Control") t(this, b);
				...
				if (this.rangeCount == h + 1) {
					if (l.config.checkSelectionRanges) if ((h = Z(this.nativeSelection, this.rangeCount - 1)) && !k.rangesEqual(h, b)) b = new r(h);
	...
	g.refresh = function (b) {
		...
		if (b) {
			...
			for (; b--;) if (!k.rangesEqual(d[b], this._ranges[b])) return false;
			return true
		}
	};
	var ba = function (b, d) {
		var h = b.getAllRanges(),
			D = false;
		b.removeAllRanges();
		for (var G = 0, P = h.length; G < P; ++G) if (D || d !== h[G]) b.addRange(h[G]);
			else D = true;
		b.rangeCount || C(b)
	};
	...
	g.containsNode = function (b, d) {
		for (var h = 0, D = this._ranges.length; h < D; ++h) if (this._ranges[h].containsNode(b, d)) return true;
		return false
	};
	g.toHtml = function () {
		var b = "";
