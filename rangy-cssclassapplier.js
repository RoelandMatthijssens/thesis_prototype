...
rangy.createModule("CssClassApplier", function (i, v) {
	...
	function m(a, b, c, d) {
		...
		if (h.isCharacterDataNode(b)) if (c == 0) {
				c = h.getNodeIndex(b);
				b = b.parentNode
			} else if (c == b.length) {
	...
	function F(a) {
		...
		return function (c, d) {
			...
			} else if (d) if ((f = e[b]) && f.nodeType == 1 && e.tagName == f.tagName && w(e, f) && A(e, f)) return f[a ?
	...
	q.prototype = {
		...
		isAppliedToRange: function (a) {
			if (a.collapsed) return !!this.getSelfOrAncestorWithClass(a.commonAncestorContainer);
				...
				for (var b = a.getNodes([3]), c = 0, d; d = b[c++];) if (!this.isIgnorableWhiteSpaceNode(d) && y(a, d) && this.isModifiable(d) && !this.getSelfOrAncestorWithClass(d)) return false;
				return true
			}
		},
		isAppliedToSelection: function (a) {
			...
			for (var b = a.length; b--;) if (!this.isAppliedToRange(a[b])) return false;
			return true
		},
